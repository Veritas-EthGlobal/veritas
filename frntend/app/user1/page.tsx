...
export default function User1Page() {
 ...
 const submit = async () => {
  ...
  setCheckpoints(0);
  // Clear previous highlights on new submission
  setHighlightedLines({});
  ...
  try {
    ...
    const answer = data;
    console.log("API call finished, received result:", answer);




    // HIGHLIGHT: Process the highlight data directly from the response
...
if (data.boundaries_json && Array.isArray(data.boundaries_json)) {
    ...
    notifyServer(matchId, 'user1', answer);
        ...
        <div className={filesContent.length > 0 ? "w-1/2" : "w-full"}>
          {submissionStatus === 'done' ? (
            ...
            <div className="text-center flex flex-col items-center justify-center h-full bg-white/5 rounded-2xl border border-white/10 p-6">
              ...
              <p className="text-gray-400 mt-2">Waiting for Player 2 and the Judge...</p>
            </div>
          ) : (
              ...
              <Card className="bg-white/5 border border-white/10 backdrop-blur-sm">
                ...
                <CardContent>
                  <Label htmlFor="match1" className="text-gray-400 mb-2 block">
                    Share this ID with Player 2 and the Judge
                  </Label>
                  <Input
                    id="match1"
